...
\begin{align*}
\mathcal{L}_\mathrm{QED} = \sum_{f=\text{fermions}}\bar{\psi_f} (i\gamma^\mu(\partial_\mu + ieQ_f A_\mu) - m_f) \psi_f - \frac{1}{4}F_{\mu\nu} F^{\mu\nu}
\end{align*}%
...
\end{align*}}%
\note{$e$ is the coupling constant and $Q_f$ is the charge.}
\end{frame}
...
\begin{itemize}
    ...
    \item <6->{Which is really a shame, since many interesting phenomena such as \textbf{confinement} is a low-energy phenomena.}
\end{itemize}}
\end{frame}
...
\onslide<1->{Given a field $\phi^M$ in Minkowski space, the \textit{partition function $Z$} is given by
\begin{align*}
    Z &= \int \mathcal{D}\phi^M \e^{\frac{i}{\hbar}S^M[\phi^M]} \\
    &\downarrow \quad \hbar=1, \quad \tau \rightarrow -it \qquad \text{imaginary time(euclidean space)!} \\
...
\note{\begin{itemize}
    ...
    \item <1->{We must also change $S$ so it is in Euclidean space.}
...
\begin{itemize}
    ...
    \item \onslide<2->{Four links at every lattice size(one for each Lorentz index). Opposite direction given by its inverse.}
...
\note{
\begin{itemize}[<+->]
    \item We calculate the action using links.
    \item We will use the Metropolis Monte Carlo method for solving the path integral and generating configurations.
    \item We have a method for handling the computations.
    \item However, some observables are problematic, and we need to apply some method of renormalization in order to retrieve sensible results..
\end{itemize}}
\end{frame}
...
\begin{itemize}
    ...
    \item \onslide<2->{In this vacua, \textbf{Instantons} are local minimums of the Yang-Mills action in Euclidean space.}
...
\end{align*}}%
\onslide<5->{\noindent%
Equally probably to have negative charge as positive,
\begin{align*}
    \expect{Q} = 0
\end{align*}}
\note{\begin{itemize}
...
\begin{itemize}
    ...
    \item \onslide<3->{The autocorrelation is given as $\Gamma(t) = \expect{(a_i - a)(a_{i+t} - a)}$ and $\tau_\mathrm{int} = \frac{1}{2} + \sum^{\infty}_{t=1}\frac{\Gamma(t)}{\Gamma(0)}$.\\}
...
\end{columns}}
% \note{\begin{itemize}
%     % \item First of all, $Q$ is a far more correlated than other quantities such as the energy.
%     \item 
% \end{itemize}}
\end{frame}
...
\end{align*}%
$q(0)$ is the \textit{source} placed at a fixed Euclidean time, and $q(n_t)$ is the \textit{sink} which at some Euclidean times $t$.%
\note{\begin{itemize}
...
\begin{itemize}
    ...
    \item <4->The topological susceptibility $\expect{\chi^{1/4}_f}$ and $N_f$ % TOPSUS AND N_F - compare ts-bootstrap and bootstrap. Re-iterate main results.
    \item <5->$\expect{Q^4}_C$ and $R$. Sensitive quantities - need large statistics.% 4th CUMULANT - quite off. Larger beta than article. Perhaps just poor statistics.
    \item <6->Topological charge correlator $\expect{q(n_t)q(0)}$ and glueball mass. % CORRELATOR AND GLUEBALL MASS
    \item <7->Statistics, autocorrelation and critical slowdown.% STATISTICS AND CRITICAL SLOWDOWN
\end{itemize}
\note{\begin{itemize}
    ...
    \item <3->$\expect{Q}$
    \item <4->$\chi^{1/4}_f$. Matches well with other papers
    \item <5->$\expect{Q^4}_C$ and $R$. Sensitive quantity, matches well of first and second moment.
    \item <6->Topological charge correlator $\expect{q(n_t)q(0)}$ and glueball mass.
    \item <7->Critical slowdown, which makes transitioning to a new, independent configuration difficult. This inhibits the gathering of statistics, and helps us explain why we for larger $\beta$ values have fewer independent gauge configurations.
\end{itemize}
}
\end{frame}

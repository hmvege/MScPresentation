...
\begin{frame}{Discretizing the path integral}
\onslide<1->{Path integral becomes
\begin{align*}
    \int \mathcal{D}\phi = \prod_{x_\mu} \dd\phi(x) = \prod_{x_\mu} \dd \phi_{x_\mu}
\end{align*}}
\onslide<2->{We integrate over each spacetime point.}
\onslide<3->{For a lattice of size $32 \times 32 \times 32 \times 32 = 2^{20}$, total number of integration points becomes $n^{2^20}$, 
which is not feasible in any of our lifetimes!\\}
\onslide<4->{\rightarrow need to use a statistical method.}
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{../figures/lattice_quarks_and_gluons.png}
\end{figure}
Quarks on points, gluons in between.
\note{The lattice is a cube in 4D.}
\end{frame}

\begin{frame}{Links}
\onslide<1->{The gluons is given by \textit{links}, $U_\mu(x) \in \SU(3)$. \\}
\onslide<2->{Complex $3\times 3$ matrices with properties of,}
...
\only<1-3>{Too large to solve on any single computer.\\}
\only<2-3>{Number of points in a lattice:
\begin{align*}
...
\end{align*}}
\only<3>{Too large to solve on any single computer.}
\only<4->{
    \begin{figure}
        \centering
        \includegraphics{../figures/parallelization-lattice/parallelization-lattice.pdf}
    \end{figure}
}
\end{frame}
...
\begin{figure}
    \centering
    \includegraphics{../figures/parallelized-lattice/parallelized-lattice.pdf}
\end{figure}
\end{frame}
...
\onslide<3->{
    \begin{figure}
        \centering
        \includegraphics{../figures/illustrations/shift/shift.pdf}
    \end{figure}
}
\end{frame}
...
%         \animategraphics[loop,controls,width=\linewidth,scale=0.7]{15}{../figures/latviz/topc_TE21/flowTime_TE21-}{0}{60}
%         % \transduration<0-16>{0}

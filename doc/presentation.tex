...
\begin{itemize}
    \item \onslide<1->{QCC vacua(i.e. gauge fields) can be classified by their topological properties.}
...
\begin{itemize}
    ...
    \item \onslide<3->{The autocorrelation is given as $\Gamma(t) = \expect{(a_i - a)(a_{i+1} - a)}$ and $\tau_\mathrm{int} = \frac{1}{2} + \sum^{\infty}_{t=1}\frac{\Gamma(t)}{\Gamma(0)}$.\\}
...
\only<1>{Some people use a banana for scale
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{../figures/banana_for_scale.jpg}
\end{figure}}
\onslide<2->{We use $t_0$
\begin{align*}
    E = \frac{a^4}{2|\Lambda|} \sum_{n\in\Lambda}\sum_{\mu,\nu} \left(F^\mathrm{clov}_{\mu\nu}(n)\right)^2 \\
\end{align*}}
\onslide<3->{$F_{\mu\nu}^\mathrm{clov}(n)$ is given by \begin{figure}
    \centering

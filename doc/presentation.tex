...
\begin{center}
    \animategraphics[loop,controls,width=0.6\linewidth,scale=1]{10}{../figures/latviz/topc_TE21_lowres/flowQ_TE21-}{00}{60}
\end{center}%
...
\begin{align*}
    q(n) = \frac{1}{32\pi^2} \epsilon_{\mu\nu\rho\sigma} \tr\left[G_{\mu\nu}(n)G_{\rho\sigma}(n)\right].
\end{align*}}%
\onslide<5->{\noindent%
...
\begin{frame}{Topological charge}
\begin{center}
    \animategraphics[loop,controls,width=0.6\linewidth,scale=1]{10}{../figures/latviz/topc_TF500_lowres/flowQ_TF500-}{00}{63}
\end{center}%
\tiny{\textit{Animation created using LatViz.}}
\end{frame}
...
\begin{itemize}
    ...
    \item <4->$\expect{Q} \neq 0$ for some ensembles $\rightarrow$ understood with autocorrelation. % CHARGE - not zero!
    \item <5->The topological susceptibility $\chi^{1/4}_f$ and $N_f$ % TOPSUS AND N_F - compare ts-bootstrap and bootstrap. Re-iterate main results.
    ...
    \item <9->The energy, the scale $t_0$ and $w_0$ were also calculated match what is found in the literature, i.e. \citet{luscher_properties_2010} and \citet{ce_non-gaussianities_2015}.% REFERENCE SCALE T0 - close to what is found by Luscher, REFERENCE SCALE W0 - close to what is found in paper
\end{itemize}
\note{\begin{itemize}

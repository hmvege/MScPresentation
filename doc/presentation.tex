...
\end{enumerate}
\husk{TODO: fikse på formattering!}
\only<4->{Goal: \textit{maintain gauge invariance.}}
...
\begin{frame}{Gauge invariance on the lattice} \husk{TODO: Dont need to show the gauge transformation!}
\begin{block}
...
The simplest gauge invariant object,\husk{TODO: drop the equatioN!?!}
\begin{align*}
...
\husk{TODO: Remove scaling and optimization slides!!}
\begin{frame}{Scaling}
We checked three types of scaling, \husk{TODO: the following 3 slides might be completely redundant - move to the end as extra material?}
\begin{itemize}[<+->]
    ...
    \item \textbf{Speedup:} defined as $S(p) = \frac{t_{N_{p}}}{t_{N_p,0}}$.
\end{itemize}
...
\note{
<1>{We run for different values for $N_\mathrm{up}$ and $N_\mathrm{corr}$ to see what gives optimizes \textbf{computational cost} and \textbf{autocorrelation}.}
<1>{The integrated autocorrelation time for topological charge $\expect{Q}$ for a lattice of size $N=16$ and $N_T=32$ with $\beta=6.0$ for combinations of $N_\mathrm{corr}\in[200,400,600]$ and $N_\mathrm{up}\in[10,20,30]$, plotted against flow time $\sqrt{8t_f}$.}
<2>{The time taking to generate 200 configurations and flowing them $N_\mathrm{flow}=250$ flow steps for a lattice of size $N=16$ and $N_T=32$, with $\beta=6.0$ for combinations of $N_\mathrm{corr}\in[200,400,600]$ and $N_\mathrm{up}\in[10,20,30]$.}
<3->{What we see is that increasing $N_\mathrm{up}$ is a cheaper alternative compared to using $N_\mathrm{corr}$}
}
\end{frame}
...
\end{align*}}
\onslide<2->{we get
\begin{align*} \husk{TODO: where is epsilon_f?!?!}
    \begin{split}
        &W_0 = V_{t_f}, \\
...
\begin{table}
    ...
    \end{tabular}
\end{table}
Charge radius of a proton: $\~0.85$ fm. \husk{Include this for perhaps the uninitiated?}
\note{The lattice sizes.}
\end{frame}

\begin{frame}{The clover field strength definition} \husk{TODO: move this to the next slide?!}
\begin{figure}
    \centering
...
\begin{frame}{Energy} \husk{TODO: fixe cutoff på bunn!!}
\vspace{-10.0pt}
\begin{figure}
    \centering
...
\begin{frame}{Autocorrelation in the energy} \husk{TODO: Maybe remove?}
\begin{figure}
    \centering
...
\begin{frame}{Topological charge} \husk{TODO: fikse formattering! må holde seg på samme plass}
\begin{itemize} \husk{TODO: winding numbers: kanskje fjerne? Gir ikke så mye ekstra!!}
...
\begin{itemize}
    ...
    \item <2->\textbf{Instantons} are local minimums to the Yang-Mills action in Euclidean space.
    \item <2->Instantons are significant because of \textit{critical slowdown} which we will return to later.
    \item <3->Measuring \textbf{topological charge} is a measure of the \textit{Winding number} of the gauge field.
\end{itemize}
}
\end{frame}

\begin{frame}{Topological charge} \husk{TODO: redundant!!}
...
\begin{frame}{Topological charge} \husk{TODO: WHY is Q expected to be zero?!}
\vspace{-5.0pt} 
\begin{figure}
    \centering
...
\begin{frame}{Additional ensembles} \husk{TODO: maybe redundant!?!?!}
\begin{table}
    \centering
    \resizebox{0.8\columnwidth}{!}{%
...
\begin{frame}{Topological charge autocorrelation} \husk{TODO: move this slide up, or move the charge dist. plots forwards?!}
\vspace{-5.0pt}
\begin{figure}
    \centering
...
\begin{frame}{Critical slowdown} \husk{TODO: move this to when I talk about instantons?!}
\husk{TODO: clean up! Make it so that stuff doesn't move when introducing new elements!}
\only<1->{\textbf{Critical slowdown} is the phenomena where we as the lattice spacing $a$ \textit{decreases} the required energy to tunnel from one state to another \textit{increase}.}
\only<2->{$\rightarrow$ many more lattice updates are required in order to have independent gauge configurations.}
\note{
\begin{itemize}
    ...
    \item <3->In the continuum it would require an infinite amount of energy to go from one instanton sector to another. Thus as we $a$ approaches the continuum, the amount of effort required to generate independent gauge configurations increases.
\end{itemize}}
\end{frame}

\begin{frame}{Topological susceptibility} \husk{TODO: snakke om divergensen her!!!}
\onslide<1->{
The \textit{topological susceptibility} is given by
\begin{align*}
    \chi_\mathrm{top}^{1/4} = \frac{1}{V^{1/4}}\expect{Q^2}^{1/4}
\end{align*}
with $V$ being the lattice volume and $\expect{Q}$ is the second momenta of the charge.}
...
\begin{itemize}
    ...
    \item $N_f$ is the number of flavors(i.e. quark species involved in $\eta'$.).
\end{itemize}}
\onslide<4->{We expect $N_f=3$.}
\note{\begin{itemize}
    ...
    \item <4->This can help us understand the "quality" of the ensemble, as we would expect to be around $N_f=3$.
\end{itemize}}
\end{frame}
...
\note{\begin{itemize}
    ...
    \item <3>Results as presented by \citet{ce_non-gaussianities_2015}, normalized by the lattice volume.
...
\begin{frame}{The topological charge correlator} \husk{TODO: fikse formattering}
\husk{TODO: ha med mer om hva vi forventer og hvorfor det ikke er sånn!}
...
\begin{frame}{The effective glueball mass} \husk{TODO: mention what a glue ball mass is!}
...
\end{figure}}
% \husk{TODO: CLEAN UP COMMENT ON FRAME!}

...
\begin{frame}{Asymptotic freedom}
    ...
    \note{
        \begin{itemize}
            ...
            \item One of the experimental proofs of QCD along with triple $\gamma$ decay and muon cross section ratio $R$. \husk{LEGGE TIL MER HER!! EVT FJERNE. OPPSUMMER MER!}
        \end{itemize}
    }
\end{frame}
...
\end{itemize}
\note{
    \husk{MORE ON WV FORMULA. CLARIFY WHAT IT IS AND WHAT IT EXPLAINS.}
    \begin{itemize}
...
\end{align*}}%
\husk{MOVE PATH INTEGRAL FIGURE TO THIS LOCATION TO ILLUSTRATE WHAT A PATH INTEGRAL IS!}
\only<3>{\begin{align*}
...
\end{align*}}%
\husk{PRESENT METROPOLIS ALGORITH AFTER SHOWING THE PATH INTEGRAL?}
\end{frame}

\begin{frame}{The path integral II}
\husk{MOVE THIS FIGURE TO BEING THE FIRST SLIDE WHEN PRESENTING PATH INTEGRALS, AS IT REALLY DOES NOT SHOW LATTICE PATH INTEGRALS}
\begin{figure}
    \centering
    \includegraphics[scale=1.0]{../figures/illustrations/lqcd/path-integral/path-integral-flattened}
\end{figure}
\note{An example of the discretized path integral, going from time $t_0$ to $t_{n}$, where the end points is taken to be equal, $x_0=x_{N_T}$. We integrate over all of space at each time $t_i$ finding the most likely position at a given time.}
\end{frame}
...
\end{itemize}}
\husk{FIX THE SLIDES HERE! FINAL <3-> is redundant!!}
\end{frame}
...
\husk{WHAT DO THE R RATIO INDICATE WHEN AWAY FROM ZERO?!}
\begin{align*}
...
\husk{CLEAN UP! REMOVE GENERAL EXPRESSION(ITS JUST MORE CONFUSING!)}
\husk{EXPLAIN HERE WHAT SOURCE/SINK IS}
...
\end{figure}
\husk{CLEAN UP COMMENT ON FRAME!}
